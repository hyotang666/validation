...
(defpackage :validation
  ...
  (:export ;; Main apis for light users.
           ...
           #:value ; To access second element of assertion, e.g. INTEGER of the
                   ; assertion (:type integer).
           #:format-args ; To access 2ndcdr sublist of assertion, e.g. ("~A
                         ; must integer" slot-name) of the assertion (:type
                         ; integer "~A must integer" slot-name)
           ))
           ...
           (lambda (&rest results)
             (values (car results)
                     (loop :for errors :in (cdr results) :by #'cddr
                           :append errors)))
         ,@(mapcar (lambda (method) `(call-method ,method nil)) primary))))
...
(defmethod update-instance ((object standard-object) &rest args)
  (loop :with initargs
        ...
        :for keys
             = (intersection (c2mop:slot-definition-initargs slot) initargs)
        :when (and keys
                   (or (null (cdr keys)) (error "Dupcated initargs ~S" keys)))
...
(defun <slot-asserts> (clause g.targets? g.obj)
  (destructuring-bind
      ((local-var slot-name) . assertions)
      clause
    (if g.targets?
        `(when (or (find ',slot-name ,g.targets?) (null ,g.targets?))
           ,#0=(next-assertion
                 (make-args :object g.obj
                            :slot-name slot-name
                            :local-var local-var
...
(defassert :key (args)
  (let ((v (gensym "CANONICALIZED")) (c (gensym "CONDITION")))
    `(handler-case
         (let ((,v (funcall ,(value args) ,(local-var args))))
       ...
       ,(if (format-args args)
            `(error nil
                     ...
                     *errors*))
            `(error (,c)
                    (push (cons ',(slot-name args) (princ-to-string ,c))
                          *errors*))))))
  ...
  `(if ,(value args)
       ...
       (push
        (cons ',(slot-name args)
              (format nil
                      ,@(or (format-args args)
                            `("must satisfies ~S but ~S" ',(value args)
                              ,(local-var args)))))
        *errors*)))
                   ...
                   :do (restart-bind ((remove-method-before-abort
                                       (lambda ()
                                         ...
                                         (invoke-restart
                                           (find-restart 'abort)))))
                         (error
                           "~S first argument must be specialized by eql-specializer, but ~S"
                           (c2mop:generic-function-name gf) specializer)))))
  ...
  (let ((names
    ...
    (dolist (slot slots)
      (unless (find slot names)
        (error "Unknown slot ~S, class ~S have only ~S" slot (type-of object)
               names)))))
...
(defmacro with-object-validation
          ((object-generate-form &key target-slots ((:test optional-clause)))
           (&rest assertions))
  ...
  (alexandria:with-unique-names (var targets clause)
    ...
    `(let ((*errors*)
       ...
       ,@(when target-slots
           `((check-type ,targets list) (check-targets ,var ,targets)))
